...
if __name__ == "__main__":
    ...
    base_path = '/home/steward/lfast/'
    sh_path = base_path + 'SHWFS/20250521/' #path pointing to folder of SH images for current night
    reference_path = os.path.join(sh_path,'223742/') #path to set of images used for pupil definition
    folder_path = os.path.join(sh_path, '223742/')
    ...
    xyr, extend_image = xyr_pupil_definition(reference_path, sh_path, redefine_pupil=True)  #Define location of pupil within SH image
    jup_crop = crop_image(extend_image,xyr)
    output_plots = False
    referenceX,referenceY,magnification,nominalSpot,rotation = lenslet_definition(reference_path, sh_path, xyr, output_plots=False, recompute_rotation=True)
#%%
    ...
    mean_surface = np.flipud(np.fliplr(np.mean(surfaces, axis=0)))
